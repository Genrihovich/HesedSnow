...
type
  ...
  private
    ...
    function SumColumn(): Currency;
    procedure RemoveRowStringGrid(CurrRow: Integer);
    function NumberRowNoEmptyText(): Integer;
...
var
  SourceCol, SourceRow, PrevSourceRow, ActiveSourceRow: Integer;
  // Источника колонка и строка координаты, предыдущее состояние строки
...
begin
  ...
  for i := 1 to StringGrid.RowCount - 1 do // строки
    ...
    begin
      if j=5 then MyExcel.Cells[i + 1, j + 1] := StringReplace(StringGrid.Cells[j, i], ',', '.', [rfReplaceAll, rfIgnoreCase])
      else
  ...
  FileNameS := DirectoryNow + 'СЛГ_' + FormatDateTime('dd.mm.yyyy hh_mm_ss',
    Now) + '.xlsx';
...
begin
  ...
  lSum.Caption := CurrToStr(SumColumn());
end;
...
begin // Очистить список
  ...
  StringGrid.RowCount := 1;
  lCount.Caption := IntToStr(0);
  lSum.Caption := IntToStr(0);
end;
    ...
    if uMyExcel.RunExcel(false, false) = true then
      ...
      begin
        ...
        begin
          ...
          tSLG.FieldByName('Kategorya').AsString :=
            MyExcel.Cells[m, StrtoInt(CollectionNameTable.Items['Категория']
            .ToString)].value;
...
function TfrmSLG.SumColumn: Currency;
var
  Rez: Currency;
  col: Integer;
begin
  Rez := 0;
  for col := 1 to StringGrid.RowCount - 1 do
  begin
    if StringGrid.Cells[5, col] <> '' then
    begin
      Rez := Rez + StrToCurr(StringGrid.Cells[5, col]);
    end;
  end;
  Result := Rez;
end;
...
begin // после перетаскивания
  ...
  begin
    ...
    begin
      ...
      // сделать активной строку эту и подсветить
      StringGrid.Row := DestRow;
    ...
    begin
      ...
      begin
        DestCol := 3;
        StringGrid.Cells[DestCol, DestRow] := MyDBGrid2.Fields[1].AsString;
...
procedure TfrmSLG.RemoveRowStringGrid(CurrRow: Integer);
var
  countRow, EmptyRow, newRow: Integer;
begin
  countRow := StringGrid.RowCount - 1; // всего строк
  EmptyRow := NumberRowNoEmptyText; // индекс чистой строки

  if EmptyRow > CurrRow then // Если чистая строка выше перемещаемой
  begin
    // Определяем куда перемещать строку
    if EmptyRow = countRow then // если последняя строка списка чистая
    begin
      newRow := EmptyRow + 1;
      StringGrid.MoveRow(CurrRow, newRow - 1);
    end;
    if EmptyRow < countRow then
    begin
      newRow := EmptyRow;
      StringGrid.MoveRow(CurrRow, newRow);
    end;
  end;
end;

function TfrmSLG.NumberRowNoEmptyText: Integer;
// Определение последней строки, что не заполнено поле с СЛГ
var
  i, countRow: Integer;
begin
  countRow := StringGrid.RowCount - 1; // всего строк
  for i := countRow DownTo 1 do
  begin
